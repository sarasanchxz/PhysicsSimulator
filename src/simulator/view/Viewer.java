...
class Viewer extends SimulationViewer {
	...
	private void drawBodies(Graphics2D g) {
		...
		for(Body b: _bodies)		{	
			int _pX = (int) ((int) _centerX + b.getPosition().getX() / _scale);
			int _pY = (int) ((int) _centerY - b.getPosition().getY() / _scale);	
			//Vector2D p= b.getPosition().scale(1/_scale).plus(center);	
			if(isVisible(b)) {			
				if (_showVectors) {
					...
					int _vX = (int) (_pX + magnitude * v.getX());	
					int _vY = (int) (_pY - magnitude * v.getY());						
					int _fX = (int) (_pX + magnitude * f.getX());				
					int _fY = (int) (_pY - magnitude * f.getY());
					// fuerza					
					drawLineWithArrow(g, _pX, _pY, _fX, _fY, 4, 4, Color.RED, Color.RED);				
					// velocidad
					drawLineWithArrow(g, _pX, _pY, _vX, _vY, 4, 4, Color.GREEN, Color.GREEN);
				}	
				
				g.setColor(_gColor.get(b.getgId()));
				g.drawString(b.getId(), _pX, _pY);	
				g.fillOval(_pX - 4, _pY - 4, 8, 8);	
